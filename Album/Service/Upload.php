 ...
 * @copyright  Copyright (c) 2021 我本奈何 (https://www.ijkxs.com)
...
class Upload
{
    ...
    public static function save($file = array())
    {
        ...
        $afterfix = DIRECTORY_SEPARATOR . $date->year . DIRECTORY_SEPARATOR . $date->month . DIRECTORY_SEPARATOR;
        $path = Typecho_Common::url(self::UPLOAD_PATH, __TYPECHO_ROOT_DIR__) . $afterfix;
        $relative_path = self::UPLOAD_PATH . $afterfix;
        ...
        for ($i = 0; $i < count($file['file']['name']); $i++) {
            ...
            if (in_array($file['file']['type'][$i], $MIME) && $file['file']['error'][$i] == 0) {
                ...
                $file_name = time() . rand(0, 1000) . $ext[$i];
                $file_path = $path . $file_name;
                $file_url = Typecho_Common::url($relative_path . $file_name, $siteUrl);
                if ($upload_Server == 'local') {
                    ...
                    $msg['url'][$i] = $file_url;//$siteUrl.str_replace( __TYPECHO_ROOT_DIR__.DIRECTORY_SEPARATOR ,'',$file_path  );
                    ...
                    if (isset($thumb[$i]['err'])) {
                        ...
                        $local_root = rtrim(__TYPECHO_ROOT_DIR__ . DIRECTORY_SEPARATOR, "\\");
                        $local_root = rtrim($local_root, "/");
                        $msg['thumb'][$i]['url'] = $siteUrl . str_replace($local_root, '', $thumb[$i]['url']);
                    }
...
//            var_dump($img_info['http_code']);
        ...
        $local_root = rtrim(__TYPECHO_ROOT_DIR__ . DIRECTORY_SEPARATOR, "\\");
        $local_root = rtrim($local_root, "/");
        $msg['url'] = $siteUrl . str_replace($local_root, '', $path . $new_name);
        $msg['server'] = $upload_Server;
        return $msg;
